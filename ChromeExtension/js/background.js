...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    ...
    var bg = chrome.extension.getBackgroundPage(); // Just here as an example that I could also get the page DOM by getting the window object
    chrome.tabs.query({ active: true }, function (tabs) {
        ...
        chrome.tabs.sendMessage(tabs[0].id, 'jacktest', function (responseFromContentScript) {
            ...
            if (responseFromContentScript) {
                // debugger;
                chrome.tabs.create({ url: 'templates/tab.html' }, function (tab) {
                    // debugger;
